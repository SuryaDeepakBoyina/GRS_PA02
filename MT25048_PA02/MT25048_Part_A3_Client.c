/*
 * MT25048_Part_A3_Client.c - Zero-Copy TCP Client
 * Roll Number: MT25048
 *
 * This client receives data from the zero-copy server.
 * Uses standard recv() as zerocopy is primarily beneficial for send operations.
 */
...
/*
 * receiver_thread - Thread function to receive zero-copy transmitted data
 */
void *receiver_thread(void *arg) {
  ...
  while (get_timestamp_sec() < end_time && !shutdown_flag) {
    ...
    /* Standard recv - zerocopy is primarily for send operations */
    ssize_t received = recv(sock_fd, aligned_buffer, aligned_size, 0);
...
int main(int argc, char *argv[]) {
  ...
  printf("=== MT25048 Part A3: Zero-Copy Client ===\n");
  printf("Server: %s:%d\n", host, port);
